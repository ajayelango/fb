...
$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
  ...
  $stmt = $conn->prepare("SELECT first_name, last_name FROM user where username = '$session_username'");
  $stmt->execute();
	
  // set the resulting array to associative
  $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
  
  $stmt2 = $conn->prepare("SELECT * FROM post where userid = '$session_username' order by posted_date desc");
  $stmt2->execute();
...
<body>
<div class="header">
<h3><?php echo $result[0]['first_name'], " ", $result[0]['last_name']; ?>
<span class="tab">
<form action="logout.php" method="POST">
<input id="logout" type="submit" name="logout" value="Logout">
</form>
<button id="wall" onClick="gotoWall()"> Wall </button>
</h3>
</div>
...
<form action="post.php" method="POST">
  <textarea id="postComment" name="postComment" rows="6" cols="70" placeholder="Post something here...">
</textarea>
